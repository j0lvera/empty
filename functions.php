...
if (!function_exists('empty_setup')) :
	...
	function empty_setup()
	{
		 ...
		 * If you're building a theme based on Empty, use a find and replace
		 * to change 'empty' to the name of your theme in all the template files.
		 */
		...
		add_theme_support(
			'custom-background',
			apply_filters('empty_custom_background_args', array(
 ...
 */
function empty_content_width()
{
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters('empty_content_width', 640);
}
add_action('after_setup_theme', 'empty_content_width', 0);
...
function empty_widgets_init()
{
	register_sidebar(array(
		'name' => esc_html__('Sidebar', 'empty'),
		'id' => 'sidebar-1',
		'description' => esc_html__('Add widgets here.', 'empty'),
...
function empty_scripts()
{
	...
	wp_enqueue_style('empty-google-preconnect', 'https://fonts.gstatic.com');

	add_filter('style_loader_tag', 'add_preconnect_attr', 10, 2);

	function add_preconnect_attr($html, $handle)
	{
		if ($handle === 'empty-google-preconnect') {
			return str_replace("rel='stylesheet'", "rel='preconnect'", $html);
		}
		return $html;
	}

	wp_enqueue_style('empty-fonts', 'https://fonts.googleapis.com/css2?family=Source+Sans+Pro:ital,wght@0,400;0,600;0,700;1,400;1,600;1,700&display=swap');
